...
func TestNormalizeOpenAIMessagesForPrompt_AssistantToolCallsAndToolResult(t *testing.T) {
	...
	if !strings.Contains(assistantContent, "<|DSML|工具调用>") {
		t.Fatalf("assistant tool history should be preserved in DSML form, got %q", assistantContent)
	}
	if !strings.Contains(assistantContent, `<|DSML|调用 name="get_weather">`) {
	...
	if !strings.Contains(prompt, "<|DSML|工具调用>") {
...
func TestNormalizeOpenAIMessagesForPrompt_AssistantMultipleToolCallsRemainSeparated(t *testing.T) {
	...
	if strings.Count(content, "<|DSML|调用 name=") != 2 {
		t.Fatalf("expected two preserved tool call blocks, got %q", content)
	}
	if !strings.Contains(content, `<|DSML|调用 name="search_web">`) || !strings.Contains(content, `<|DSML|调用 name="eval_javascript">`) {
...
func TestNormalizeOpenAIMessagesForPrompt_AssistantNilContentDoesNotInjectNullLiteral(t *testing.T) {
	...
	if !strings.Contains(content, "<|DSML|工具调用>") {
...
func TestNormalizeOpenAIMessagesForPrompt_CanonicalizesStandaloneAssistantToolMarkupContent(t *testing.T) {
  ...
  <！/DSML！tool_calls>`,
	...
	for _, want := range []string{
		"<|DSML|工具调用>",
		`<|DSML|调用 name="Bash">`,
		`<|DSML|参数 name="command"><![CDATA[lsof -i :4321 -t]]></|DSML|参数>`,
		`<|DSML|参数 name="description"><![CDATA[Verify port 4321 is free]]></|DSML|参数>`,
		"</|DSML|工具调用>",
	} {
