...
func parseSettingsUpdateRequest(req map[string]any) (*config.AdminConfig, *config.RuntimeConfig, *config.ResponsesConfig, *config.EmbeddingsConfig, *config.DeepSeekConfig, *config.AutoDeleteConfig, *config.CurrentInputFileConfig, *config.ThinkingInjectionConfig, map[string]string, error) {
	var (
		adminCfg        *config.AdminConfig
		runtimeCfg      *config.RuntimeConfig
		respCfg         *config.ResponsesConfig
		embCfg          *config.EmbeddingsConfig
		deepSeekCfg     *config.DeepSeekConfig
	...
	if raw, ok := req["deepseek"].(map[string]any); ok {
		cfg := &config.DeepSeekConfig{}
		if v, exists := raw["file_base_url"]; exists {
			cfg.FileBaseURL = strings.TrimRight(strings.TrimSpace(fmt.Sprintf("%v", v)), "/")
		}
		if err := config.ValidateDeepSeekConfig(*cfg); err != nil {
			return nil, nil, nil, nil, nil, nil, nil, nil, nil, err
		}
		deepSeekCfg = cfg
	}
	...
	return adminCfg, runtimeCfg, respCfg, embCfg, deepSeekCfg, autoDeleteCfg, currentInputCfg, thinkingInjCfg, aliasMap, nil
}
