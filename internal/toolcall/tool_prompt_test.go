...
func TestBuildToolCallInstructions_ExecCommandUsesCmdExample(t *testing.T) {
	...
	if !strings.Contains(out, `<|DSML|调用 name="exec_command">`) {
		t.Fatalf("expected exec_command in examples, got: %s", out)
	}
	if !strings.Contains(out, `<|DSML|参数 name="cmd"><![CDATA[pwd]]></|DSML|参数>`) {
...
func TestBuildToolCallInstructions_ExecuteCommandUsesCommandExample(t *testing.T) {
	...
	if !strings.Contains(out, `<|DSML|调用 name="execute_command">`) {
		t.Fatalf("expected execute_command in examples, got: %s", out)
	}
	if !strings.Contains(out, `<|DSML|参数 name="command"><![CDATA[pwd]]></|DSML|参数>`) {
...
func TestBuildToolCallInstructions_BashUsesCommandAndDescriptionExamples(t *testing.T) {
	...
	for _, block := range blocks {
		if !strings.Contains(block, `<|DSML|参数 name="command">`) {
			t.Fatalf("expected every Bash example to use command parameter, got: %s", block)
		}
		if strings.Contains(block, `<|DSML|参数 name="path">`) || strings.Contains(block, `<|DSML|参数 name="content">`) {
			t.Fatalf("expected Bash examples not to use file write parameters, got: %s", block)
		}
		if strings.Contains(block, `<|DSML|参数 name="description">`) {
			sawDescription = true
		}
	}
	if !sawDescription {
	...
	if strings.Contains(out, `<|DSML|调用 name="Read">`) {
...
func TestBuildToolCallInstructions_ExecCommandLongScriptUsesCmd(t *testing.T) {
	...
	for _, block := range blocks {
		if !strings.Contains(block, `<|DSML|参数 name="cmd">`) {
			t.Fatalf("expected exec_command examples to use cmd parameter, got: %s", block)
		}
		if strings.Contains(block, `<|DSML|参数 name="command">`) || strings.Contains(block, `<|DSML|参数 name="path">`) || strings.Contains(block, `<|DSML|参数 name="content">`) {
...
func TestBuildToolCallInstructions_WriteUsesFilePathAndContent(t *testing.T) {
	...
	for _, block := range blocks {
		if !strings.Contains(block, `<|DSML|参数 name="file_path">`) || !strings.Contains(block, `<|DSML|参数 name="content">`) {
			t.Fatalf("expected Write examples to use file_path and content, got: %s", block)
		}
		if strings.Contains(block, `<|DSML|参数 name="path">`) {
...
func TestBuildToolCallInstructions_AnchorsMissingOpeningWrapperFailureMode(t *testing.T) {
	...
	if !strings.Contains(out, "Никогда не пропускай открывающий тег <|DSML|工具调用>") {
		t.Fatalf("expected explicit missing-opening-tag warning, got: %s", out)
	}
	if !strings.Contains(out, "Ошибка 3 - отсутствует открывающая обертка") {
...
func TestBuildToolCallInstructions_RejectsEmptyParametersInPrompt(t *testing.T) {
	...
	for _, want := range []string{
		"Не выводи плейсхолдеры, пустые или состоящие только из пробелов параметры.",
		"Если обязательное значение параметра неизвестно, спроси пользователя на китайском языке или ответь нормально вместо пустого вызова инструмента.",
		"Никогда не вызывай их с пустой командой.",
...
func TestBuildToolCallInstructions_UsesPositiveTagPunctuationAlphabet(t *testing.T) {
	out := BuildToolCallInstructions([]string{"Bash"})
	want := `Для пунктуации тегов используй ASCII < > / = " и полуширинную вертикальную черту |.`
	if !strings.Contains(out, want) {
...
func findInvokeBlocks(text, name string) []string {
	...
	for {
		...
		end := strings.Index(remaining, `</|DSML|调用>`)
		if end < 0 {
			return blocks
		}
		end += len(`</|DSML|调用>`)
